...
static inline bool simd_compare_1_64(const uint8_t* hay, const uint8_t* pat, size_t len)
{
    ...
    if (mask0 != 0xFFFFFFFFu) return false;
...
static inline bool simd_compare_any(const uint8_t* hay, const uint8_t* pat, size_t len)
{
    ...
    if (len <= 64) return simd_compare_1_64(hay, pat, len);
...
std::vector<size_t> bmh_simd_avx2_all_extended(
    const uint8_t* hay, size_t hay_len,
    const uint8_t* pat, size_t pat_len)
{
    std::vector<size_t> result;
    if (pat_len == 0) return result;
    if (hay_len < pat_len) return result;
